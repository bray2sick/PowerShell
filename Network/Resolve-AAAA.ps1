<#
.SYNOPSIS
    Performs an NSLookup for the AAAA record of a specified domain using a specified DNS server.

.DESCRIPTION
    This script performs an NSLookup for the AAAA record of a specified domain using a specified DNS server. The user can choose from a list of predefined DNS servers or enter a custom DNS server IP address. The script then displays the NSLookup results for the specified domain.

.EXAMPLE
    PS C:\> .\Network\Resolve-AAAA.ps1
    Select a DNS server:
    ...
    Name:    google.com
    Address:  2404:6800:4015:803::200e
...
try {
    ...
    Write-Host " "
    $nslookupResult = nslookup -type=AAAA $Domain $selectedDnsServer | Out-String

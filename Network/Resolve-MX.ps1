<#
.SYNOPSIS
    Performs an MX record lookup for a specified domain using a specified DNS server.

.DESCRIPTION
    This script performs an MX record lookup for a specified domain using a specified DNS server. The user can choose from a list of predefined DNS servers or enter a custom DNS server IP address. The script then displays the MX record lookup results for the specified domain.

.EXAMPLE
    PS C:\> .\Network\Resolve-MX.ps1
    Select a DNS server:
...
try {
    ...
    Write-Host " "
    $mxLookupResult = nslookup -type=mx $Domain $selectedDnsServer | Out-String
    Write-Host $mxLookupResult
}
catch {
    Write-Host "An error occurred while performing the MX lookup." -ForegroundColor Red

<#
.SYNOPSIS
    Performs an NSLookup for a specified domain using a specified DNS server.

.DESCRIPTION
    This script performs an NSLookup for a specified domain using a specified DNS server. The user can choose from a list of predefined DNS servers or enter a custom DNS server IP address. The script then displays the NSLookup results for the specified domain.

.EXAMPLE
    PS C:\> .\Network\Resolve-NS.ps1
    Select a DNS server:
    ...
    Name:    google.com
    Addresses:  2404:6800:4015:803::200e
              142.250.70.174
...
try {
    ...
    Write-Host " "
    $nslookupResult = nslookup $Domain $selectedDnsServer | Out-String
